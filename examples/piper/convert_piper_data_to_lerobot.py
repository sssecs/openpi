...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def get_step(self, episode_id, step):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            head_image_path = root[f'/camera/color/DcwHeadCamera'][start_index].decode('utf-8')
            head_image_path = os.path.join(os.path.dirname(dataset_path), head_image_path)
            head_image = cv2.imread(head_image_path)
            if head_image is not None:
                head_image = cv2.cvtColor(head_image, cv2.COLOR_BGR2RGB)  # Convert to RGB
                head_image = cv2.resize(head_image, (256, 256))
            else:
                head_image = np.zeros((256, 256, 3), dtype=np.uint8)

            head_image_path = root[f'/camera/color/DcwWristCamera'][start_index].decode('utf-8')
            head_image_path = os.path.join(os.path.dirname(dataset_path), head_image_path)
            wrist_image = cv2.imread(head_image_path)
...
def create_lerobot_dataset_from_raw(dataset_dir, repo_id):
    ...
    for i in episode_ids:
        for j in range(all_episode_len[i]):
            ...
            frame_data = {
                "image": sample['wrist_image'],
                "head_image": sample['head_image'],
                "state": sample['state'],
